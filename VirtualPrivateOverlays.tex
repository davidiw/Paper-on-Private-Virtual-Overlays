...
\begin{document} \title{Addressing the P2P Bootstrap Problem for Small Overlay
Networks}
...
overlays, such as Google Talk and Live Journal Talk, and
Symphony*\footnote{Symphony* is used to ensure anonymity.}, a structured
...
implementations: a XMPP (Jabber)~\cite{xmpp} and Symphony*~\cite{brunet}.
...
Symphony* provides generic P2P abstractions as well as an implementation of the
...
Symphony* overlay with NAT-constrained nodes.
...
Symphony*VPN~\cite{socialvpn} relies on a dedicated bootstrap overlay that runs
on PlanetLab.  Using the techniques described in this paper, Symphony*VPN could
...
\\ \hline
Symphony*~\cite{brunet} &
Symphony* like OpenDHT is a freely available DHT running on PlanetLab, though
...
overlays are included.  From this list, we chose to extend Symphony* and XMPP
to support private overlay bootstrapping.  Symphony* provides a structured P2P
...
structured overlays (including Symphony* and Pastry) in effect act as $O(1)$
systems, self-organizing links that establish all-to-all connectivity among
peers.  Symphony* explicitly supports all-to-all connectivity, though in some
...
which in Symphony* is configurable at run time.
...
identifiers of the form ``symphony*://P2P\_ID:PORT'', where each peer has a
...
extended Symphony*'s transport library to support treating an existing overlay
...
they exchange an identifier of the form ``symphony*://P2P\_ID''.
...
bootstrapping required only an additional 700 lines of code to Symphony* and no
...
overheads should be expected in using Symphony* and XMPP to bootstrap an
overlay.  Rather than an extensive experiment overly focused on overheads of
Symphony* and XMPP, this experiment is primarily focused on the feasibility of
...
transparently available to them if they bootstrap into a private Symphony*
...
correlated to timeouts in Symphony*, which employs a mixture of events and
...
We extended Symphony* to support bootstrapping small-scale overlays from
another Symphony* overlay as well as XMPP.  XMPP was chosen as readily
supported the key features required for bootstrapping small-scale P2P overlays.
The Symphony* solution offers interesting insights. In particular, we have
