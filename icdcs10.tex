...
\author{\IEEEauthorblockN{David Isaac Wolinsky and Kyungyong Lee and Tae Woong Choi and P. Oscar Boykin and Renato Figueiredo} 
...
Our results are presented in Figured~\ref{fig:single_join}.  It appears that
both the estimated, simulated, and real results all have similar slopes.
Thus it should be expected that the time necessary to form a connection to
a private overlay is effectively the time it takes to connect to the public
overlay, perform a Dht get on the public overlay, and then the time of
performing standard connection to the private overlay.  Though we expected
nothing unusual during the security handshaking, it is interesting to note
how all results have nearly identical slope and that securing the overlay
does not connecting with it slower.
